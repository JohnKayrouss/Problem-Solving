...
function _getMiddle(word) {
	let even = word.length % 2 == 0;
	return (function getWordMiddle() {
		if (even) {
			let half = word.split("").length / 2;
			return `${word[half - 1]}${word[half]}`;
		}
		if (!even) {
			let middle = Math.floor(word.split("").length / 2);
			return `${word[middle]}`;
		}
	})();
}

console.log(_getMiddle("testing"));
